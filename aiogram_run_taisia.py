...
from db import dai_authors, dai_olympiads, dai_lang, dai_tags, add_task, top_search #МЯУМЯУ
...
async def ask_next_param(message: Message, user_id: int):
    """
    Спрашивает следующий параметр из списка выбранных
    """
...
async def perform_search(message: Message, user_id: int):
    ...
    try:
        ...
        results = top_search(all_params.values())
        ...
        if not results:
            ...
            await show_results(message, results, values_dict)
