...
from db import task_po_id, dai_authors, dai_olympiads, dai_lang, dai_tags
from db import add_task, top_search, vivod_na_check, DB
...
def make_params_keyboard(selected_params):
    ...
    for param_key, param_label in all_params.items():
        ...
        buttons.append([InlineKeyboardButton(text=button_text,
                       callback_data=f"select_{param_key}")])

    # Кнопки действий
    buttons.append([InlineKeyboardButton(
        text="Начать поиск", callback_data="start_search")])
    buttons.append([InlineKeyboardButton(
        text="Отмена", callback_data="cancel_search")])
...
def make_single_choice_keyboard(
        items,
        chosen_item,
        back_callback,
        page=0,
    ...
    for idx, item in enumerate(current_items, start=start):
        if item == chosen_item:
            ...
            button_text = f"⬜ {item}"
        buttons.append([InlineKeyboardButton(text=button_text,
                       callback_data=f"choose_olympiad_{idx}")])
    ...
    if page > 0:
        nav_buttons.append(
            InlineKeyboardButton(
                text="Назад",
                callback_data=f"olympiad_page_{
                    page - 1}"))
    nav_buttons.append(InlineKeyboardButton(
        text=f"{page + 1}/{total_pages if total_pages > 0 else 1}", callback_data="noop"))
    ...
    lower_row = [
        InlineKeyboardButton(
            text="Готово", callback_data="done_olympiad"), InlineKeyboardButton(
            text="Пропустить", callback_data="skip_olympiad"), InlineKeyboardButton(
...
def multi_choice_keyboard(
    ...
    for idx, item in enumerate(current_items, start=start):
        if item in chosen_items:
            ...
            button_text = f"⬜ {item}"
        buttons.append([InlineKeyboardButton(text=button_text,
                                             callback_data=f"choose_multi_{param_type}_{idx}")])
    ...
    if page > 0:
        nav_buttons.append(
            InlineKeyboardButton(
                text="Назад",
                callback_data=f"multi_page_{param_type}_{
                    page - 1}"))
    nav_buttons.append(InlineKeyboardButton(
        text=f"{page + 1}/{total_pages if total_pages > 0 else 1}", callback_data="noop"))
    if page < total_pages - 1:
    ...
    if chosen_items:
        lower_row.append(
            InlineKeyboardButton(
                text="Готово",
                callback_data=f"done_multi_{param_type}"))
    lower_row.append(
        InlineKeyboardButton(
            text="Пропустить",
            callback_data=f"skip_{param_type}"))
    lower_row.append(
        InlineKeyboardButton(
            text="Назад",
...
async def handle_search_callbacks(callback: CallbackQuery):
    ...
    if data == "done_olympiad":
        if "olympiad" not in search_values[user_id]:
            await callback.answer("Пожалуйста, выбери олимпиаду или нажми «Пропустить»!")
            return
        page_key = f"olympiad_{user_id}"
    ...
    if data.startswith("multi_page_"):
        ...
        if len(parts) >= 4:
            ...
            await callback.message.edit_reply_markup(
                reply_markup=multi_choice_keyboard(
                    items, chosen, param_type, back_callback, new_page
                )
            )
        await callback.answer()
        return
    ...
    # пропуск для любого типа параметров (единичный для олимпиады, мульти для
    # тэгов, авторов и языков и текстовый ввод для года и названия)
...
# Спрашивает пользователя о значении следующего параметра
async def next_param(message: Message, user_id: int):
    ...
    if current_param == "olympiad":
        ...
        await message.answer(
            ...
            f"Ты выбрал параметр: олимпиада\n\n"
            f"Выбери олимпиаду из списка (можно только одну):",
            parse_mode="Markdown",
            reply_markup=make_single_choice_keyboard(
                items, chosen, back_callback, current_page
            )
        )
        return
    ...
    if current_param in ["authors", "tags", "language"]:
        ...
        await message.answer(
            ...
            parse_mode="Markdown",
            reply_markup=multi_choice_keyboard(
                items, chosen, current_param, back_callback, current_page
            )
        )
        return
    ...
    param_info = {
        "name": (
            "название задачи",
            "Введи название задачи (можно часть слова)"),
        "year": (
            "год",
            "Введи год (например: 2024)")}

    param_name, prompt_text = param_info.get(
        current_param, (current_param, "Введи значение"))

    search_step[user_id] = f"asking_{current_param}"
    search_skip_keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="Пропустить",
                    callback_data=f"skip_{current_param}"
                )
            ],
            [
                InlineKeyboardButton(
                    text="Назад",
                    callback_data=(
                        "back_to_previous" if current_index > 0
                        else "back_to_params"
                    )
                )
            ]
    ...
    await message.answer(
        ...
        f"{prompt_text}",
        parse_mode="Markdown",
        reply_markup=search_skip_keyboard
    )
...
async def perform_search(message: Message, user_id: int, state: FSMContext):
    ...
    try:
        ...
        if not results:
            await message.answer(
                "Задачи не найдены\n\n"
                "Попробуй выбрать другие параметры или использовать /search для нового поиска",
            )
        else:
            ...
            for i, task in enumerate(results[:5], 1):
                ...
                if task.get('authors'):
                    authors_str = ", ".join(
                        task['authors']) if isinstance(
                        task['authors'],
                        list) else task['authors']
                ...
                if task.get('language'):
                    langs_str = ", ".join(
                        task['language']) if isinstance(
                        task['language'],
                        list) else task['language']
                    response += f"Язык: {langs_str}\n"
                if task.get('tags'):
                    tags_str = ", ".join(task['tags'][:5]) if isinstance(
                        task['tags'], list) else task['tags']
                    response += f"Тэги: {tags_str}\n"
                response += "\n"
            await message.answer(response)
            await message.answer(
                "Введи <b>ID задачи</b> с клавиатуры, чтобы получить её условие и ответ:"
            )
...
async def process_task_id_input(message: Message, state: FSMContext):
    ...
    if not task:
        await message.answer("Задача с таким ID не найдена в нашей базе данных. Попробуй ещё раз:")
        return
    ...
    if not task_data:
        ...
        isinstance(task_data, str) and len(task_data) > 30
        and not task_data.startswith('http') and ' ' not in task_data
    ):
    ...
    if not answer_data:
        ...
        isinstance(answer_data, str) and len(answer_data) > 30
        and not answer_data.startswith('http') and ' ' not in answer_data
    ):
        await message.answer_document(
        ...
        await message.answer(
            f"<b>Ответ на задачу (нажми, чтобы посмотреть):</b>\n"
            f"<tg-spoiler>{answer_data}</tg-spoiler>"
        )

    await message.answer("Для нового поиска введи /search")
...
def make_pagination_keyboard(items, page, prefix, per_page=10):
    ...
    if page > 0:
        nav_row.append(
            InlineKeyboardButton(
                text="Назад",
                callback_data=f"list_nav:{prefix}:{
                    page - 1}"))
    ...
    if prefix != "menu":
        inline_keyboard.append([InlineKeyboardButton(
            text="⬅Назад к категориям", callback_data="list_back_to_menu")])
...
async def full_task(message: Message, task: dict):
    ...
    if task.get('authors'):
        response += f"Авторы: {task['authors']}\n"
    if task.get('olympiad'):
        response += f"Олимпиада: {task['olympiad']} ({task['year'] or '—'})\n"
    if task.get('language'):
        response += f"Язык: {task['language']}\n"
    if task.get('tags'):
        response += f"Тэги: {task['tags']}\n"
...
async def process_item_click(callback: CallbackQuery):
    ...
    cursor.execute(
        f"SELECT id, title, olympiad, year FROM tasks "
        f"WHERE {column} LIKE ? AND status = 'approved'",
        (f"%{selected_value}%",)
    )
    ...
    task_buttons_data = []
    for t in db_tasks:
        btn_text = t['title'].strip() if t['title'] and t['title'].strip(
        ) else f"Олимпиада: {t['olympiad'] or 'Олимпиада'} ({t['year'] or '---'})"
    ...
    await callback.message.edit_text(
        f"Задачи по запросу «{selected_value}»:",
        reply_markup=kb,
        parse_mode="Markdown"
    )
    await callback.answer()
...
def files_keyboard():
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="Загрузить файл", callback_data="file")], [
                InlineKeyboardButton(
                    text="Ввести текстом", callback_data="text")]])
    return keyboard
# кнопка скипа
...
def noskip_onetag_keyboard(tags_db, chosen_tags, page=0, one_page=10):
    ...
    for i, tag in enumerate(tags, start=start):
        if tag in chosen_tags:
            ...
            chosen = tag
        buttons.append([InlineKeyboardButton(
            text=chosen, callback_data=f"one_{i}")])
    row = []
    if page > 0:
        row.append(
            InlineKeyboardButton(
                text="Назад",
                callback_data=f"ol_page_{
                    page - 1}"))
    row.append(
        InlineKeyboardButton(
            text=f"{
                page + 1}/{total}",
            callback_data="dlyakrasoty"))
    if page < total - 1:
        row.append(
            InlineKeyboardButton(
                text="Вперед",
...
def tags_keyboard(tags_db, chosen_tags, page=0, one_page=10):
    ...
    for i, tag in enumerate(tags, start=start):
        if tag in chosen_tags:
            ...
            chosen = tag
        buttons.append([InlineKeyboardButton(
            text=chosen, callback_data=f"tag_{i}")])
    ...
    lower_row = [
        InlineKeyboardButton(
            text="Готово", callback_data="done"), InlineKeyboardButton(
            text="Пропустить", callback_data="skip"), InlineKeyboardButton(
                text="Нет в списке", callback_data="custom_tag")]
    buttons.append(lower_row)
    return back_keyboard(buttons)
...
async def handle_tags(callback: CallbackQuery, user_id: int):
    ...
    if data.startswith("page_"):
        ...
        await callback.message.edit_reply_markup(
            reply_markup=tags_keyboard(
                tags_db, answers[user_id][info], page
            )
        )
        await callback.answer()
        return
    if data.startswith("ol_page_"):
        ...
        answers[user_id]['page'] = page
        await callback.message.edit_reply_markup(
            reply_markup=noskip_onetag_keyboard(
                tags_db, answers[user_id]['olympiad'], page
            )
        )
        await callback.answer()
        return
    if step.get(user_id) == 3.1:
        if data.startswith('one_'):
            ...
            await callback.message.edit_reply_markup(None)
            await callback.message.answer(
                'Введи название олимпиады.',
                reply_markup=onlyback_keyboard()
            )
            await callback.answer()
        elif data == 'done':
            if not answers[user_id].get(
                    'olympiad') and not answers[user_id].get('custom_olympiad'):
                await callback.answer('Пожалуйста, выбери олимпиаду или нажми "Нет в списке".')
                return
            answers[user_id]['olympiad'] = answers[user_id]['olympiad'] if answers[user_id].get(
                'olympiad') else answers[user_id].get('custom_olympiad')
        ...
        if tag in answers[user_id][info]:
            ...
            await callback.answer(f'Тэг "{tag}" удалён.')
        else:
            answers[user_id][info].append(tag)
            await callback.answer(f'Тэг "{tag}" добавлен.')
        await callback.message.edit_reply_markup(
            reply_markup=tags_keyboard(
                tags_db, answers[user_id][info]
            )
        )
    elif data == 'custom_tag':
        ...
        await callback.message.edit_reply_markup(None)
        await callback.message.answer(
            f'Введи свои варианты для "{info}" через запятую, если их несколько.',
            reply_markup=onlyback_keyboard()
        )
        await callback.answer()

    elif data == "done":
        all = list(set(answers[user_id].get(
            custom, []) + answers[user_id].get(info, [])))
        answers[user_id][info] = all
        if not answers[user_id][info]:
            await callback.answer('Выбери хотя бы один тэг или нажми "Пропустить".')
            return
        ...
        if next == 2:
            await callback.message.answer(
                'Шаг 2. Добавь год олимпиады. '
                'Если не помнишь, нажми "Пропустить", '
                'однако тогда ты пропустишь также Шаг 3 с указанием олимпиады.',
            ...
            await callback.message.answer(
                'Шаг 6. Как ты хочешь добавить задачу?',
...
async def cmd_start(message: Message):
    await message.answer(
        'Привет! Это база лингвистических задач, '
        'в которой ты можешь найти задания по автору, '
        'олимпиаде, году и т. п. Пиши /add, чтобы добавить задачу, '
        'или /search, чтобы найти задачу.'
    )
...
async def cmd_add(message: Message):
    ...
    step[user_id] = 0
    await message.answer(
        'Шаг 0. Введи название задачи. Если не хочешь указывать, нажми "Пропустить".',
        reply_markup=skip_keyboard()
    )
...
async def callbacks(callback: CallbackQuery):
    ...
    if data == "skip":
        ...
        if data == "file":
            ...
            await callback.message.edit_reply_markup(None)
            await callback.message.answer(
                'Пожалуйста, отправь файл с задачей.',
                reply_markup=onlyback_keyboard()
            )
        elif data == "text":
            step[user_id] = 6.3
        ...
        if data == 'file':
            ...
            await callback.message.edit_reply_markup(None)
            await callback.message.answer(
                'Пожалуйста, отправь файл с ответом.',
                reply_markup=onlyback_keyboard()
            )
        elif data == 'text':
            step[user_id] = 7.3
...
async def get_tags1(message: Message, user_id: int):
    ...
    step[user_id] = 1.1
    await message.answer(
        'Шаг 1. Выбери авторов задачи из списка. '
        'Если не хочешь указывать, нажми "Пропустить".',
        reply_markup=tags_keyboard(tags_db, answers[user_id]['authors'])
    )
...
async def get_tags3(message: Message, user_id: int):
    ...
    step[user_id] = 3.1
    await message.answer(
        'Шаг 3. Выбери олимпиаду, на которой встретилась задача, из списка. '
        'Этот шаг обязателен, так как ты указал год.',
        reply_markup=noskip_onetag_keyboard(tags_db, answers[user_id]['olympiad'])
    )
...
async def get_tags4(message: Message, user_id: int):
    ...
    step[user_id] = 4.1
    await message.answer(
        'Шаг 4. Выбери из списка язык, которому посвящена задача. '
        'Если не хочешь указывать, нажми "Пропустить".',
        reply_markup=tags_keyboard(tags_db, answers[user_id]['language'])
    )
...
async def get_tags5(message: Message, user_id: int):
    ...
    step[user_id] = 5.1
    await message.answer(
        'Шаг 5. Выбери тэги, связанные с задачей. '
        'Если не хочешь указывать, нажми "Пропустить".',
        reply_markup=tags_keyboard(tags_db, answers[user_id]['tags'])
    )
...
async def answer_message(message: Message):
    ...
    if step[user_id] == 1:
        ...
        answers[user_id]['custom_authors'] = [tag.strip()
        ...
        await message.answer(
            f'Ты указал олимпиаду "{text.strip()}". '
            'Если это верно, нажми "Готово". Если ты хочешь указать другую, выбери ее из списка.'
        )
        ...
        answers[user_id]['custom_language'] = [tag.strip()
                                               for tag in text.split(',')]
        ...
        if message.document is not None:
            ...
            step[user_id] = 7.1
            await message.answer(
                'Шаг 7. Как ты хочешь добавить ответ на задачу?',
                reply_markup=files_keyboard()
            )
        else:
            await message.answer(
