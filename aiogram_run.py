...
async def handle_search_callbacks(callback: CallbackQuery): #Обрабатывает все коллбеки от кнопок поиска
    ...
    #пропуск для любого типа параметров (единичный для олимпиады, мульти для тэгов, авторов и языков и текстовый ввод для года и названия)
...
async def next_param(message: Message, user_id: int): #Спрашивает пользователя о значении следующего параметра
    ...
    search_skip_keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="Пропустить", callback_data=f"skip_{current_param}")],
        [InlineKeyboardButton(text="Назад", callback_data="back_to_previous" if current_index > 0 else "back_to_params")]
    ])
...
async def process_task_id_input(message: Message, state: FSMContext):
    ...
    elif isinstance(task_data, str) and len(task_data) > 30 and not task_data.startswith('http') and ' ' not in task_data:
        await message.answer_document(document=task_data, caption="<b>Файл с условием задачи:</b>")
    else:
    ...
    elif isinstance(answer_data, str) and len(answer_data) > 30 and not answer_data.startswith('http') and ' ' not in answer_data:
        await message.answer_document(document=answer_data, caption="<b>Файл с ответом на задачу:</b>")
    else:
        await message.answer(
...
async def full_task(message: Message, task: dict):
    ...
    if task.get('tags'): response += f"Тэги: {task['tags']}\n"
...
async def cmd_listasks(message: Message):
    ...
    await message.answer("Выбери категорию для просмотра задач:", reply_markup=keyboard, parse_mode="Markdown")
...
async def process_back_to_menu(callback: CallbackQuery):
    ...
    await callback.message.edit_text("Выбери категорию для просмотра задач:", reply_markup=keyboard, parse_mode="Markdown")
    await callback.answer()
