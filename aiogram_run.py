...
search_values = {}    # значения параметров (словарь: ключ -> список значений или значение для олимпиады)
...
def make_back_keyboard(): #новая клавиатура только с кнопкой "Назад"
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
...
def make_multi_choice_keyboard(items, chosen_items, param_type):
    #Клавиатура для множественного выбора (авторы, теги, языки)
    buttons = []
    for item in items:
...
async def handle_search_callbacks(callback: CallbackQuery): #Обрабатывает все коллбеки от кнопок поиска
    ...
    #обработаем возврат к выбору параметров
    ...
    if data.startswith("choose_multi_"):
        # Формат: choose_multi_authors_Иванов
        ...
        if len(parts) >= 4:
            ...
            await callback.message.edit_reply_markup(
                reply_markup=make_multi_choice_keyboard(items, search_values[user_id][param_type], param_type)
            )
        return
    ...
    if data.startswith("done_multi_"):
        ...
        # Если ничего не выбрано, оставляем пустой список (потом будет пропущено)
        if param_type not in search_values[user_id]:
            search_values[user_id][param_type] = []
        search_index[user_id] += 1
        await callback.message.edit_reply_markup(None)
...
async def ask_next_param(message: Message, user_id: int): #Спрашивает пользователя о значении следующего параметра
    ...
    if current_param in ["authors", "tags", "language"]:
        ...
        await message.answer(
            ...
            parse_mode="Markdown",
            reply_markup=make_multi_choice_keyboard(items, chosen, current_param)
        )
        return
...
async def perform_search(message: Message, user_id: int):
    ...
    for param, value in values_dict.items():
        if param == "olympiad":
            #для олимпиады value - это строка или None, превращаем в список
...
async def show_full_task(message: Message, task: dict):
    ...
    #выводим условие
    await message.answer("*Условие задачи:*", parse_mode="Markdown")
    ...
    #выводим ответ
    await message.answer("*Ответ на задачу:*", parse_mode="Markdown")
