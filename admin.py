...
async def pending_tasks(
    ...
    buttons.append([InlineKeyboardButton(text="Одобрить",
                                         callback_data="dobro"),
...
async def column(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        f"Введите новое значение для поля *{column}*:",
        parse_mode="Markdown"
    )
    await callback.answer()
...
async def edit_value(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"✅ Поле *{column}* обновлено на: {new_value}",
        parse_mode="Markdown"
    )
    ...
    for task in tasks[start:end]:
        ...
        buttons.append([
            InlineKeyboardButton(
                text=f"{title} (ID: {task['id']})",
                callback_data=f"mod_{task['id']}"
            )
        ])
    row = []
    if page > 0:
        row.append(
            InlineKeyboardButton(
                text="Назад",
                callback_data="mod_prev"))
    if page < total - 1:
        row.append(
            InlineKeyboardButton(
                text="Вперед",
                callback_data="mod_next"))
    if row:
        buttons.append(row)
    buttons.append([
...
@admin.callback_query(F.data.in_({"dobro",
                                  "otklon",
                                  "edit",
                                  "pred",
                                  "next",
                                  "exit"}) | F.data.startswith("mod_"))
async def callback(callback: CallbackQuery, state: FSMContext):
    ...
    if data == "mod_prev":
        if user_id in mdata:
            ...
            if row:
                buttons.append(row)
            buttons.append([InlineKeyboardButton(
                text="Выйти", callback_data="mod_exit")])
    ...
    if data == "mod_next":
        if user_id in mdata:
            mdata[user_id]['index'] = min(
                len(mdata[user_id]['tasks']) - 1, mdata[user_id]['index'] + 1)
            tasks = mdata[user_id]['tasks']
            page = mdata[user_id]['index']
            total = (len(tasks) + 9) // 10
            start = page * 10
    ...
    if data == "dobro":
        ...
        if sender:
            try:
                await callback.bot.send_message(
                    sender,
                    f"Ваша задача (ID: {task_id}) была одобрена и добавлена в базу данных."
                )
            except Exception as e:
                print(
                    f"Ошибка при отправке сообщения пользователю {sender}: {e}")
        return
    elif data == "otklon":
        ...
        if sender:
            try:
                await callback.bot.send_message(
                    sender,
                    f"Ваша задача (ID: {task_id}) была отклонена."
                )
            except Exception as e:
                print(
                    f"Ошибка при отправке сообщения пользователю {sender}: {e}")
