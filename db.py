...
def task_po_author(author):
    ...
    cursor.execute(
        "SELECT id, language FROM tasks WHERE authors LIKE ? AND status = 'approved'",
        (f"%{author}%",)
    )
...
def task_po_tag(tag):
    ...
    cursor.execute(
        "SELECT id, language FROM tasks WHERE tags LIKE ? AND status = 'approved'",
        (f"%{tag}%",)
    )
...
def task_po_lang(lang):
    ...
    cursor.execute(
        "SELECT id, language FROM tasks WHERE language LIKE ? AND status = 'approved'",
        (f"%{lang}%",)
    )
